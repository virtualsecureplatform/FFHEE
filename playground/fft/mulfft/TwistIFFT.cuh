...
namespace SPCULIOS{
    ...
    __global__ void TwistIFFTlvl1(double* const res, const uint32_t* a){
            const unsigned int tid = threadIdx.x;
            const unsigned int bdim = blockDim.x;

            __shared__ double buff[TFHEpp::DEF_N];
            TwistMulInvert<uint32_t,TFHEpp::DEF_N>(buff,a,twistlvl1);
            IFFT<TFHEpp::DEF_Nbit-1>(buff,tablelvl1);

            #pragma unroll
            for(int i = tid;i<TFHEpp::DEF_N;i+=bdim) res[i] = buff[i];
            __threadfence();
        }
